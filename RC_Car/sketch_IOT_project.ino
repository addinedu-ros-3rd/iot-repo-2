...
#define trigger A3  // ultrasonic sencor Trigger pin
 
#define servo A5  // servo enable pin
 
int Set=15;  // obstacle distance
int distance_L, distance_F, distance_R;  // left, foward, right distance by super sonic sensor
...
void setup(){
  ...
  pinMode(in4, OUTPUT);  // declare asoutput for L298 Pin in4 
...
// Serial.print("Distance Foward = ");Serial.println(distance_F);

//if Right Sensor and Left Sensor are at White color then it will call forword function
// Serial.print("R_S = ");Serial.println(digitalRead(R_S));
// Serial.print("L_S = ");Serial.println(digitalRead(L_S)); 
...
if((digitalRead(R_S) == 0)&&(digitalRead(L_S) == 0)){
  ...
  // if both Sensors are Black then it will call stop function
  Stop();
}
delay(10);
}

// It generates a PWM signal to direct the servo motor to the destination, and adjusts the position of the servo motor according to the angle value.
// The position of the servo motor can be set to any desired angle.
...
long Ultrasonic_read(){
  ...
  return time / 29 / 2;  // return distance about obstacle
}
